...
class SchemaOperations {
    ...
    static generateColumnRules(column) {
        var _a;
        let columnRules = [];
        columnRules.push(column.is_nullable === 'YES' ? 'nullable' : 'required');
        if (column.Foreign && column.Foreign.length > 0) {
            columnRules.push('exists:' + column.Foreign.join(','));
            return columnRules;
        }
        ...
        switch (true) {
            ...
            case type.includes('char'):
                ...
                columnRules.push((_a = 'max:' + column.character_maximum_length) !== null && _a !== void 0 ? _a : '255');
                break;
            case type === 'text':
                columnRules.push('string');
                break;
